    ...
    if not api_key:
        ...
        with st.spinner("Generating personalized summary..." if language == "English" else "جاري إنشاء الملخص..."):
            try:
                ...
                st.subheader("📊 Lifestyle Snapshot" if language == "English" else "📊 لمحة عن أسلوب الحياة")
                exercise_score = 5 if activity_level == "Low" else (7 if activity_level == "Moderate" else 9)
                df = pd.DataFrame(
                    {"Score": [6, exercise_score, 7, 5]},
                    index=["Diet awareness", "Exercise", "Monitoring", "Stress/Sleep"] if language == "English" else
                          ["الوعي الغذائي", "التمارين", "المراقبة", "الضغط/النوم"]
                )
                st.bar_chart(df)
